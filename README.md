...
Для данной задачи на виртуальной машине было увеличено количество ядер (по сравнение с первым заданием), все представленные далее программы написаны с учетом возможности использования любого количества процессов, однако тестировалось только на максимально возможном для виртуальной машины - 2.
## 2.1

Листинг программы можно найти в `src/main.cpp`, в `src/simple_main.cpp` показан пример работы алгоритма для одной пары сгенерированных векторов заданного размера (`VECTOR_SIZE 7`), а в `src/main_with_array.cpp` продемонстрирована корректность работы алгоритма с помощью вывода массива попарно перемноженных координат заданной пары векторов.
В конце данного отчета приведен только `src/main.cpp`.

Продемонстрируем работу `src/main.cpp`:

```cmd
polina@polina-VirtualBox:~/Prog/parallel_2/src$ mpic++ main.cpp -g -O3 -o main.o
polina@polina-VirtualBox:~/Prog/parallel_2/src$ mpiexec -n 2 ./main.o
```
...
## 2.2
Напомним результаты оптимизации (без MPI) задачи интегрирования на отрезке (исходный код был немного доработан для корректной работы openmp, листинг программы содержится в `src2/main_pragma.cpp`):
...
|--------------------|--------------------|--------------------|--------------------|--------------------|
|        100         |      0.000238      |   420576.273610    |       665861       |     73.066667      |
|        1000        |      0.000444      |   2250670.699869   |      1246147       |     73.066667      |
|       10000        |      0.004275      |   2339092.647249   |      12001244      |     73.066667      |
|       100000       |      0.052852      |   1892074.661190   |     148402691      |     73.066667      |
|      1000000       |      0.349333      |   2862597.299110   |     980917281      |     73.066667      |
...
|   SPLIT SEGMENTS   |        TIME        |       GFLOPS       |  PROCESSOR CLOCKS  |  OPERATION RESULT  |
|--------------------|--------------------|--------------------|--------------------|--------------------|
|        100         |      0.000000      |        inf         |       102492       |     78.279964      |
|        1000        |      0.000104      |   9616586.688721   |       410602       |     73.066667      |
|       10000        |      0.001097      |   9118638.040222   |      3197372       |     73.066667      |
|       100000       |      0.010938      |   9142464.278106   |      30831237      |     73.066667      |
|      1000000       |      0.133319      |   7500821.921314   |     374471844      |     73.066667      |

По сравнению с оптимизацией без MPI видно заметное повышение производительности. 
